...
class ApplicationController extends Controller
{
            ...
            elseif($request->check_step == 'academic'){
                ...
                if($request->has('institution_name') && $request->filled('institution_name') ) {
                    if(sizeof($request->institution_name) == sizeof($request->institution_address ) && 
                        ...
                        return response()->json(['status'=>'Nok','msg'=>'failed, Kindly complete fields for Other Qualifications'],401);   
                    }
                }  
    ...
    public function updateProfile(Request $request){
        ...
        DB::table('applicants')->where('email', $request->email)->update(
            ['phone' => $request->phone, 'matric_number' =>  $request->email,'updated_at' => Carbon::now() ]);
